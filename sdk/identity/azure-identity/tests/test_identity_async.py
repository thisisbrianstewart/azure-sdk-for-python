...
from helpers import AsyncMockTransport, mock_response, Request, async_validating_transport, wrap_in_future
...
async def test_imds_credential_cache():
    ...
    credential = ImdsCredential(transport=AsyncMockTransport(send=wrap_in_future(mock_send)))
    ...
    for status_code in (404, 429, 500):
        ...
        try:
            await ImdsCredential(
                transport=AsyncMockTransport(send=wrap_in_future(mock_send), sleep=wrap_in_future(lambda _: None))
            ).get_token("scope")
